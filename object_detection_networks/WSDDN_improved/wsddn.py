...
class WeaklySupervisedDetection(tf.keras.Model):
    ...
    def __init__(self, backbone):
        ...
        #note: add background class
        self.fc8c = Dense(hp.num_classes+1, name='block8_fcc')


        #detection branch layers: add background class as well
        self.fc8d = Dense(hp.num_classes+1, name='block8_fcd')
    ...
    def call(self, image, labels, spatial_reg=False):
        ...
        y2 = original_rois[:,3]*(backbone_pre_pooling_output.shape[0])
    ...
    def difference_triplet_loss(self, fc_detect_out, rois_feature):
        ...
        return hp.triplet_loss_weight*(total_triplet_loss+hp.epsilon)
    ...
    def similarity_loss(self, fc_detect_out, rois_feature):
        ...
        for i in range(hp.num_classes + 1):
            ...
            class_filtered_features = tf.linalg.normalize(class_filtered_features, ord='euclidean', axis=1)[0]


            distance_matrix = tf.reduce_sum((tf.expand_dims(class_filtered_features, 1)-tf.expand_dims(class_filtered_features, 0))**2,2)
    ...
    def spatial_regularizer(scores, labels, fc7, rois):
        ...
        return hp.spatial_reg_weight*(regularizer_sum/(hp.num_classes))+hp.epsilon
    ...
    def l2_regularizer(self):
        ...
        for layer in all_layers:
            ...
            #skip layers with pretrained weights
