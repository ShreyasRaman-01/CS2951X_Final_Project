...
class DatasetCreator:
    ...
    def create_datasets(self, path_to_data):
        ...
        class_label = [1 for _ in range(self.num_classes)]+[0]
    ...
    def train_step(image_batch, label_batch, spatial_reg=False):
        ...
        with tf.GradientTape() as tape:
            ...
            for image, label in zip(image_batch, label_batch):
                ...
                loss_value = loss_value + hp.cross_entropy_loss_weight*model.crossentropy_loss(tf.expand_dims(output, axis=0), tf.cast(label, tf.float32)) +  triplet_loss + model.l2_regularizer() + spatial_regularizer_output
