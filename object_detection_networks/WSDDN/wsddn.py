...
def bounding_box_iou(boxes1, boxes2):
    ...
    intersect_x1 = tf.reduce_max(b1_x1, b2_x1, axis=1)
    intersect_y1 = tf.reduce_max(b1_y1, b2_y1, axis=1)

    intersect_x2 = tf.reduce_min(b1_x2, b2_x2,axis=1)
    intersect_y2 = tf.reduce_min(b1_y2, b2_y2,axis=1)
...
class WeaklySupervisedDetection(tf.keras.Model):
    ...
    def spatial_regularizer(scores, labels, fc7, rois):
        ...
        for k in range(hp.num_classes):
            ...
            fc7_output_mask = tf.repeat(tf.expand_dims(region_ious, axis=1), (1,fc7.shape[1]))
